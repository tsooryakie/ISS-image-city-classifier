...
in an 80/10/10 split, respectively. It takes a random sample of images from each class and moves them
...
def split_dataset(root_dir: str, mode: str) -> None:
    ...
    random.seed(
        42
    )  # RNG is seeded to ensure that same images are used for the split (ensures fairness in experiment)
    ...
    for i in range(len(classes_available)):
        ...
        images_to_export = random.sample(
            images_in_class, int(len(images_in_class) * 0.2)
        )
        ...
        for j in range(len(images_to_export)):
            shutil.move(
                root_dir + classes_available[i] + "/" + images_to_export[j],
                root_dir.replace("train", mode)
                + classes_available[i]
                + "/"
                + images_to_export[j],
            )
